  ...
  setSelected = (date, event, keepInput, monthSelectedIn) => {
    ...
    if (
      ...
      if (selectsRange) {
        ...
        const filteredSelected = multipleSelected.filter(
          (date) => !isSameDay(date, changedDate)
        );
        if (filteredSelected.length === multipleSelected.length) {
          onChange(multipleSelected.concat([changedDate]), event);
        } else {
          onChange(filteredSelected, event);
        }
      } else {
